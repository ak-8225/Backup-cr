 ...
 * @returns {Promise<Object>} - Object containing collegeOrder, notes, and likedColleges
 */
export async function getUserCollegeData(phone) {
  try {
    if (!phone) return { collegeOrder: [], notes: {}, likedColleges: [] };
    ...
    if (docSnap.exists()) {
      ...
      // Ensure likedColleges exists even if it's not in the database
      if (!data.likedColleges) {
        data.likedColleges = [];
      }
      return data;
    } else {
      return { collegeOrder: [], notes: {}, likedColleges: [] };
    }
  } catch (error) {
    console.error("Error getting user college data:", error);
    return { collegeOrder: [], notes: {}, likedColleges: [], error: error.message };
  }
}
 ...
 * @param {Array} likedColleges - Array of college IDs that are liked by the user
 * @returns {Promise<Object>} - Result of the operation
 */
export async function saveUserCollegeData(phone, collegeOrder, notes, likedColleges = []) {
  try {
    ...
    const data = {
      ...
      likedColleges: Array.isArray(likedColleges) ? likedColleges : []
    };
